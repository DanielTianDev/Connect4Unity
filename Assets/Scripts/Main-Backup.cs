 ...
 * 
 * Connect 4 Ai assignment 5 using minimax algorithm, alpha beta pruning, transposition tables of previously computed values from static evaluation, as well as loading in precomputed
 * transposition table from a file to save computation time. This program can play with a depth of 9 within a reasonable amount of time (10 seconds average for the first 10 moves)
 * 
 * */
public class Main : MonoBehaviour {
    ...
    public int MAX_DEPTH = 5, MAX_DEPTH2 = 3;
    ...
    private void Update()
    {
        ...
        {
            int colIndex = FindBestMove2(Board, 666);
            int temprow = PlacePiece(colIndex, Board, MINIMIZER);

            var go = Instantiate(minimizerPrefab, PieceSpawnLocations[colIndex].transform.position, minimizerPrefab.transform.rotation);
            go.GetComponent<Piece>().SetCol(VisualBoard[temprow, colIndex].transform.position);
    ...
    async Task<int> FindBestMove(int[,] board, int testParam) //multi threaded
    {
        ...
        for(int col = 0; col < 7; col++)
        {
            if (Board[0, col] == 0) //check if empty
            {
                ...
                tasks.Add(Task.Run(() => Minimax(tempBoard, 0, dynamicDepth, false, int.MinValue, int.MaxValue, testParam)));
    ...
    int FindBestMove2(int[,] board, int testParam) //multi threaded
    {
        ...
        for (int col = 0; col < 7; col++)
        {
            if (Board[0, col] == 0) //check if empty
            {
                ...
                int val = Minimax(tempBoard, 0, MAX_DEPTH2, false, int.MinValue, int.MaxValue, testParam);
                if (val < bestVal)
                {
    ...
    int Minimax(int[,] board, int currentDepth, int maxDepth, bool isMax, int alpha, int beta, int testparam)
    {
        ...
        if(testparam == 666)
        {
            score = Evaluate(board, isMax, testparam)  - currentDepth;
        }
        else
        {
            lock (minimaxLock)
            {
                ...
                {
                    score = Evaluate(board, isMax, testparam);
            ...
            for (int col = 3; val <= 3; val++) //traverse all cells
            {
                if (val == 0)
                {
                    if (Board[0, col] == 0) //check if board empty
                    {
                        ...
                        PlacePiece(col, tempBoard, MAXIMIZER);
                        int result = Minimax(tempBoard, currentDepth + 1, maxDepth, !isMax, alpha, beta, testparam); //call minimax recursively and choose the max value
                ...
                else
                {
                    if (Board[0, col - val] == 0) //check left
                    {
                        ...
                        int result = Minimax(tempBoard, currentDepth + 1, maxDepth, !isMax, alpha, beta, testparam);
                        if (result > best) best = result;
                        if (best > alpha) alpha = best;
                        if (alpha >= beta) break;
                    }
            ...
            for (int col = 3; val <= 3; val++)
            {
                if (val == 0)
                {
                    if (Board[0, col] == 0) //check if board empty
                    {
                        int[,] tempBoard = CopyBoard(board); //make the move
                        PlacePiece(col, tempBoard, MINIMIZER);
                        int result = Minimax(tempBoard, currentDepth + 1, maxDepth, !isMax, alpha, beta, testparam); //call minimax recursively and choose the mininum value
                        if (result < best) best = result;
    ...
    int Evaluate(int[,] board, bool isMax, int playa) //Scoring function - There should be 69 possible ways to win on an empty board.
    {
        evalCount++;
